...
public class ConstrainedTaskPlacer implements TaskPlacer {
  ...
  public List<TTaskLaunchSpec> assignTask(THostPort nodeMonitorAddress) {
    ...
    if (taskSpec != null) {
      String temp = taskSpec.getTaskId() + "%" +"0";
      taskSpec.setTaskId(temp);
      ...
      if (taskSpecs.size()==1){
    	  String temp = taskSpecs.get(0).getTaskId() + "%" +"1";
    	  taskSpecs.get(0).setTaskId(temp);
    	  this.launchedConstrainedTasks.add(taskSpecs.get(0));
      }
  ...
  private List<TTaskLaunchSpec> getUnconstrainedTask(THostPort nodeMonitorAddress) {
	...
	int i = 0;
	int maxsize  =  this.unlaunchedUnconstrainedTasks.size();
    while (i < maxsize){
	    if (!this.launchedConstrainedTasks.contains(spec)) {
			unlaunchedUnconstrainedTasks.remove(0);
			return Lists.newArrayList(spec);
	    }else{
	    	unlaunchedUnconstrainedTasks.remove(0);
	    	if (unlaunchedUnconstrainedTasks.size()>0){
	    		spec =unlaunchedUnconstrainedTasks.get(0);
	    	}
	    }
	    i += 1;
    }
  ...
  public void adjust(){
	  for (THostPort nodeMonitor: unlaunchedConstrainedTasks.keySet()){
		  List<TTaskLaunchSpec> taskSpecs = unlaunchedConstrainedTasks.get(nodeMonitor);
		  for (TTaskLaunchSpec task : taskSpecs){
			  unlaunchedUnconstrainedTasks.add(task);
		  }
	  }	  
  }
}
