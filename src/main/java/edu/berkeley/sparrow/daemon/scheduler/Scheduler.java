 ...
 */
class constraintsOff {
	public long time;
	public boolean on;
	public int numtasks;
	public boolean cons;

	public constraintsOff(Long time, Boolean on, int num, Boolean cons) {
		this.time = time;
		this.on = on;
		this.numtasks =num;
		this.cons = cons;
...
public class Scheduler {
	...
	private ConcurrentMap<String, constraintsOff> constraintsRequests;
	private ConcurrentMap<String, TSchedulingRequest> requetMap;
	...
	public void initialize(Configuration conf, InetSocketAddress socket) throws IOException {
		...
		constraintsRequests = Maps.newConcurrentMap();
		requetMap = Maps.newConcurrentMap();
	...
	public void handleJobSubmission(TSchedulingRequest request) throws TException {
		...
		if (constrained) {
			...
			constraintsRequests.put(requestId, new constraintsOff(start, false, request.getTasks().size(), true));
		} else {
			if (request.isSetProbeRatio()) {
			...
			constraintsRequests.put(requestId, new constraintsOff(start, false, request.getTasks().size(),false));
		}
		requestTaskPlacers.put(requestId, taskPlacer);
		requetMap.put(requestId, request);

		for (String k : constraintsRequests.keySet()) {
			constraintsOff constraintsrequest = constraintsRequests.get(k);
			synchronized(constraintsrequest){
		    		if (!constraintsrequest.on && start - constraintsrequest.time > 437.5 && constraintsrequest.numtasks > 0 && constraintsrequest.cons){
		    			AUDIT_LOG.info(Logging.auditEventString("!!!!!!!!!resampling"));
					constraintsrequest.on = true;
					TSchedulingRequest requestk = requetMap.get(k);
		    			Resampling resampling = new Resampling(k, defaultProbeRatioUnconstrained);
		    	    		Map<InetSocketAddress, TEnqueueTaskReservationsRequest> enqueueTaskReservationsRequests;
		    	    		enqueueTaskReservationsRequests = resampling.getEnqueueTaskReservationsRequests(
		    	        		requestk, k, backends, address, constraintsrequest.numtasks);
					...
					TaskPlacer taskPlacertemp = requestTaskPlacers.get(k);
					synchronized(taskPlacertemp){
						taskPlacertemp.adjust();
    ...
    constraintsOff constraintsrequest  =  constraintsRequests.get(requestId);
    
    constraintsrequest.numtasks -= 1;
