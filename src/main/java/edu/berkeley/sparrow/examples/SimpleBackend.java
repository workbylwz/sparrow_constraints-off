...
public class SimpleBackend implements BackendService.Iface {
  ...
  private class TaskRunnable implements Runnable {
    ...
    public void run() {
      ...
      String temp  = this.taskId.getTaskId().split("%")[0];
      int constraintson = Integer.valueOf(this.taskId.getTaskId().split("%")[1]);
      this.taskId.setTaskId(temp);
      try {
    	if (constraintson == 0){
    		Thread.sleep(taskDurationMillis);
        }else{
		int value = 0;
        	value = 2 * taskDurationMillis;
        	Thread.sleep(value);
        }
      } catch (InterruptedException e) {
